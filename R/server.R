...
server <- function(input, output, session) {
  ...
  observe({
    country_chosen    <- input$destination_country
    choices_cities    <- choices_cities[[country_chosen]]$city_name %>% stringr::str_sort()
    
    # Can use character(0) to remove all choices
    if (is.null(country_chosen))
      choices_cities <- character(0)
    
    # Can also set the label and select items
    updateSelectizeInput(session, "destination_city",
  ...
  observeEvent(input$do, {
    ...
    city_data <- data_values %>% filter(city_name == city_chosen)
